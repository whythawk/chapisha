...
T = TypeVar("T", bound="ContributorRoles")
...
class ContributorRoles(str, Enum):
    ...
    @classmethod
    def from_text(self: T, text: str | None = None) -> T | None:
        for member in self.__members__.keys():
            if text and member in text:
                return self(member)
        return None
...
class Contributor(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Name of a person, organisation, etc. that played a secondary role - such as an editor - in the creation of the work.",
    )
    terms: str = Field(
        None,
        description="Information about copyright held by the rights-holder and their terms-of-use for their contribution to the creative work. Formatted as you wish it to appear.",
    )
...
class DublinCoreMetadata(BaseModel):
    ...
    language: str = Field(
        default="en", description="Specify the language of the creative work. Two letter code defined by ISO 639-1."
    )
    rights: str = Field(
    ...
    creator: list[str] = Field(
        ...,
        description="Name of a person, organisation, etc. responsible for the creation of the work. May be more than one.",
    )
    contributor: list[Contributor] = Field(
        None,
        description="Name of a person, organisation, etc. that played a secondary role - such as an editor - in the creation of the work. May be more than one.",
    )
    isodate: date = Field(
    ...
    publisher: str = Field(
        None, description="Name of a person, organisation, etc.  responsible for making the creative work available."
    )
...
class WorkMetadata(DublinCoreMetadata):
    ...
    long_description: Optional[str] = Field(
        None, alias="long-description", description="The pitch, or jacket-cover, description of the creative work."
    )
    long_rights: Optional[list[str]] = Field(
        None,
        alias="long-rights",
        description="Information about copyright held in and over the creative work. Formatted as you wish it to appear.",
    )
    work_uri: Optional[HttpUrl] = Field(None, alias="work-uri", description="The URI for your creative work.")
    publisher_uri: Optional[HttpUrl] = Field(
        None, alias="publisher-uri", description="The URI for the publisher of your creative work."
    )
    word_count: Optional[int] = Field(None, alias="word-count", description="Total word count of your creative work.")
