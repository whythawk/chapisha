...
class Matter(BaseModel):
    ...
    content: Optional[Union[FrontMatter, BodyMatter, BackMatter]] = Field(
        None, description="Material to be included in the work. If none provided, is a chapter by default."
    )
    title: Optional[str] = Field(
        None,
        description="Matter title. If none provided, takes the term for the matter content type. Must be provided if a chapter.",
    )

    @model_validator(mode="after")
    def assure_matter_is_related(self):
        p, c, t = self.partition, self.content, self.title
        if c is None:
